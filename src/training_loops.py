...
def train_loop(training_data, validation_data, model, model_weights, optimizer,
               opt_state, loss_fn, checkpoint_path, epochs):
...
def validate(validation_data, model, weights) -> float:
    ...
    for batch in validation_data:
        ...
        input_data = jnp.array(input_data)
        target_data = jnp.array(target_data)
        loss_v, _ = forward(weights, model, input_data, target_data)
        total_loss += loss_v
        num_batches += 1
